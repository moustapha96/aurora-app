...
function CollectionsImagePreview({ src, alt }: { src: string; alt: string }) {
  ...
  const handleAISuggest = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("personal-ai-suggest", {
        body: { module: "collections", currentContent: watch("description") }
      });
          ...
          <div>
            ...
            {watch("image_url") && (
              <div className="relative mt-2">
                <CollectionsImagePreview src={watch("image_url")} alt={t("photo")} />
                <Button
                  type="button"
                  variant="destructive"
                  size="icon"
